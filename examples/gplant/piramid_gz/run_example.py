"""
Example script for doing inverting synthetic gz data GPlant
"""
...
utils.set_logfile('piramid_example.log')
...
model = []
model.append({'x1':-500, 'x2':500, 'y1':-500, 'y2':500, 'z1':0, 'z2':1000,
              'value':1000})
model.append({'x1':-1000, 'x2':1000, 'y1':-1000, 'y2':1000, 'z1':1000, 'z2':2000,
               'value':1000})
model.append({'x1':-1500, 'x2':1500, 'y1':-1500, 'y2':1500, 'z1':2000, 'z2':3000,
               'value':1000})
model.append({'x1':-2000, 'x2':2000, 'y1':-2000, 'y2':2000, 'z1':3000, 'z2':4000,
               'value':1000})
...
# Now calculate gz contaminate the data with gaussian noise
error = 0.01
data = {}
field = 'gz'
data[field] = synthetic.from_prisms(model, x1=-5000, x2=5000, y1=-5000, y2=5000,
...
# Plot the synthetic data generated
pylab.figure()
pylab.suptitle(r'Synthetic $g_z$ data with %g mGal noise' % (error))
field = 'gz'
# Plot the data generated
pylab.axis('scaled')
levels = vis.contourf(data[field], 10)
vis.contour(data[field], levels, color='k')
#cb = pylab.colorbar()
#cb.set_label('mGal')
...
seeds = []
#seeds.append(gplant.get_seed((0, 0, 3500), 1000, mesh))
seeds.append(gplant.get_seed((1000, 1000, 3500), 1000, mesh))
seeds.append(gplant.get_seed((1000, -1000, 3500), 1000, mesh))
seeds.append(gplant.get_seed((-1000, -1000, 3500), 1000, mesh))
seeds.append(gplant.get_seed((-1000, 1000, 3500), 1000, mesh))
seeds.append(gplant.get_seed((0, 0, 2500), 1000, mesh))
#seeds.append(gplant.get_seed((500, 500, 2500), 1000, mesh))
#seeds.append(gplant.get_seed((500, -500, 2500), 1000, mesh))
#seeds.append(gplant.get_seed((-500, 500, 2500), 1000, mesh))
#seeds.append(gplant.get_seed((-500, -500, 2500), 1000, mesh))
#seeds.append(gplant.get_seed((0, 0, 1500), 1000, mesh))
#seeds.append(gplant.get_seed((0, 0, 500), 1000, mesh))
...
# Run the inversion
results = gplant.grow(data, mesh, seeds, compactness=10**(-1), power=3,
...
pylab.axis('scaled')
levels = vis.contour(data[field], levels=10, color='b', label='Data')
