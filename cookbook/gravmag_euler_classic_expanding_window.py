...
model = [
    ft.mesher.Prism(-1500, -500, -1500, -500, 1000, 2000, {'magnetization':2}),
    ft.mesher.Prism(500, 1500, 500, 2000, 1000, 2000, {'magnetization':2})]
...
# in SI
tf = (ft.utils.nt2si(ft.gravmag.prism.tf(xp, yp, zp, model, inc=-45, dec=0))
      + baselevel)
# Calculate the derivatives using FFT
xderiv = ft.gravmag.fourier.derivx(xp, yp, tf, shape)
yderiv = ft.gravmag.fourier.derivy(xp, yp, tf, shape)
zderiv = ft.gravmag.fourier.derivz(xp, yp, tf, shape)
...
index = 3
results = ft.gravmag.euler.expanding_window(xp, yp, zp, tf, xderiv, yderiv, zderiv,
    index, ft.gravmag.euler.classic, center, 500, 5000)
