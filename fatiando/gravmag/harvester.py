...
def sow(seeds, mesh, mu=0., delta=0.0001, reldist=False):
    """
    Generate a set of :class:`~fatiando.gravmag.harvester.SeedPrism` from a
    list of points and physical properties.

    This is the preferred method for generating seeds! We strongly discourage
    using :class:`~fatiando.gravmag.harvester.SeedPrism` directly unless you
    ...
    * seeds : list
        List of :class:`~fatiando.gravmag.harvester.SeedPrism`
...
class DMPrism(object):
    ...
    :meth:`~fatiando.gravmag.harvester.DMPrism._effect_of_prism`. This method
    ...
    To build a prism data module for the gravity anomaly::
        ...
        class DMPrismGz(DMPrism):
            ...
            def _effect_of_prism(self, index, props):
                return fatiando.gravmag.prism.gz(self.xp, self.yp, self.zp,
                    [self.mesh[index]])
...
class DMPrismGz(DMPrism):
    ...
    See :class:`~fatiando.gravmag.harvester.DMPrism` for details.

    **WARNING**: It is not recommended that you use this class directly. Use
    function :func:`~fatiando.gravmag.harvester.wrapdata` to generate data
    modules instead.
...
class SeedPrism(object):
    ...
    **It is highly recommended** that you use function
    :func:`~fatiando.gravmag.harvester.sow` to generate the seeds
...
def harvest(dms, seeds, iterate=False):
    ...
    * iterate : True or False
    ...
    .. note:: See the docs of this module, :mod:`fatiando.gravmag.harvester`,
