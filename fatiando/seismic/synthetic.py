...
* :func:`fatiando.seismo.synthetic.vel_from_image`
    Create a 2D wave velocity model from an image file

* :func:`fatiando.seismo.synthetic.shoot_straight2d`
    Shoot straight rays along a 2D wave velocity model and register the travel
    times.
...
import fatiando
from fatiando.seismo import _traveltime as traveltime
...
def vel_from_image(fname, vmax, vmin):
    ...
    The image is converted to gray scale and the gray intensity is used to set
    a velocity value for each pixel.
...
def shoot_cartesian_straight(model, src_n, rec_n, type='circle', rec_span=45.):
    ...
    * dictionary with travel time data
    ...
    log.info("Shooting straight rays through 2D velocity model")
    ...
    for l in xrange(ntraveltimes):
        ...
        for i in xrange(sizey):
            for j in xrange(sizex):
                tt = traveltime.straight2d(float(1./model[i][j]), float(j),
                                           float(i), float(j + 1),
                                           float(i + 1), float(src_x),
                                           float(src_y), float(rec_x),
                                           float(rec_y))
                data['traveltime'][l] += tt
    data['error'] = numpy.zeros(ntraveltimes)
    log.info("  rays shot=%d  (%.2f s)" % (ntraveltimes, time.time() - tstart))
    return data
