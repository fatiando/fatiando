/* **************************************************************************
   ...
   Set of linear algebra functions for use in inverse problems. All matrices are
   1-dimensional double arrays and are accessed using the macro POS in linalg.h
 ...
 */
int solve_linsys_lu(double *lu, int dim, int *permut, double *y, double *x)
{
    int i, k;
    double *b, tmp;
    ...
    for(i=0; i<dim; i++)
    {
        /* Permute b (y copied into b) */
        tmp = b[i];
        b[i] = b[permut[i]];
        b[permut[i]] = b[i];

        /* By now b[i] = y_permuted[i] */
